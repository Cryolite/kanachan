...
class EpsilonGreedy(TensorDictModuleBase):
    def __init__(
            self, *, epsilon_first: float, epsilon_last: float, annealing_steps: int,
            device: torch.device, dtype: torch.dtype) -> None:
        if epsilon_first < 0.0 or epsilon_first > 1.0:
            raise ValueError('`epsilon_first` must be a real number within the range [0.0, 1.0].')
        if epsilon_last > epsilon_first:
            raise ValueError('`epsilon_last` must be less than or equal to `epsilon_last`.')
        if epsilon_last < 0.0:
            raise ValueError('`epsilon_last` must be a non-negative real number.')
        if annealing_steps < 0:
            raise ValueError('`annealing_steps` must be a non-negative integer.')
        if annealing_steps == 0 and epsilon_first != epsilon_last:
            raise ValueError(
                '`annealing_steps == 0` is possible only if `epsilon_first == epsilon_last`.')

        super().__init__()

        self.register_buffer(
            'epsilon_first', torch.tensor(epsilon_first, device=device, dtype=dtype))
        self.register_buffer(
            'epsilon_last', torch.tensor(epsilon_last, device=device, dtype=dtype))
        self.register_buffer('epsilon', torch.tensor(epsilon_first, device=device, dtype=dtype))
    ...
    def forward(self, td: TensorDict) -> TensorDict:
        ...
        random_action = action_mask.float().multinomial(1).squeeze(1).to(dtype=torch.int32)
        assert random_action.dim() == 1
        assert random_action.size(0) == batch_size
        assert (candidates[torch.arange(batch_size), random_action] < NUM_TYPES_OF_ACTIONS).all().item()
    ...
    def step(self, steps: int=1) -> None:
        ...
        self.epsilon = epsilon_first * ((self.annealing_steps - self._step) / self.annealing_steps)
