...
def piecewise_linear_encoding(
        value: float | int | Tensor, minimum: float, maximum: float, dimension: int,
    ...
    if minimum > maximum:
        raise ValueError(f'{minimum} > {maximum}')
    if dimension <= 0:
        raise ValueError(dimension)

    interval = (maximum - minimum) / dimension
    intervals = torch.arange(
        1, dimension + 1, requires_grad=False, device=torch.device('cpu'), dtype=torch.float64)
    intervals *= interval
    intervals += minimum
    result = torch.full_like(intervals, value)
    result = torch.where(result >= intervals, torch.ones_like(result), torch.zeros_like(result))
...
def get_grading_point(
        room: int, game_style: int, grade: int, ranking: int, score: int, celestial_scale: int,
        all_celestial: bool) -> float:
    if room < 0 or 4 < room:
    ...
    if grading_point is None:
        raise RuntimeError('An invalid combination of the room and the grade.')
    if grade == 15:
