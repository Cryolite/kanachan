...
from torch.distributed import (
    init_process_group, is_initialized, get_world_size, get_rank, ReduceOp, all_reduce)
...
def _training(
        *, device: torch.device, dtype: torch.dtype, amp_dtype: torch.dtype,
        training_data: Path, num_workers: int, num_samples: int, celestial_scale: int,
        network_tdm: TensorDictSequential, batch_size: int, gradient_accumulation_steps: int,
        loss_function: str, max_gradient_norm: float, optimizer: Optimizer,
        scheduler: LRScheduler | None, snapshot_interval: int, summary_writer: SummaryWriter,
    ...
    data_loader = DataLoader(
        ...
        pin_memory=(num_workers >= 1), drop_last=True)
    ...
    for data in data_loader:
        ...
        def get_target(_data: TensorDict) -> Tensor:
            ...
            for i in range(batch_size):
                ...
                target = torch.zeros(4, device=torch.device('cpu'), dtype=dtype)
                for seat in range(4):
                    grade = grades[i][seat]
                    score = scores[i][seat]
                    ranking = 0
                    ...
                    target[seat] = get_grading_point(
                        room[i], game_style[i], grade, ranking, score, celestial_scale,
                        all_celestial)
        ...
        if batch_count % gradient_accumulation_steps == 0:
            ...
            nn.utils.clip_grad_norm_(
                network_tdm.parameters(), max_gradient_norm, error_if_nonfinite=False)
            if grad_scaler is None:
                optimizer.step()
            else:
        ...
        if rank == 0 and last_snapshot is not None and num_samples - last_snapshot >= snapshot_interval:
...
def _main(config: DictConfig) -> None:
    world_size, rank, local_rank, device, dtype, amp_dtype = _config.device.validate(config)
    ...
    if config.initial_model_prefix is not None:
        ...
        if config.initial_model_index is None:
            for child in os.listdir(config.initial_model_prefix):
                match = re.search(
                    '^(?:encoder|decoder|optimizer|lr-scheduler)(?:\\.(\\d+))?\\.pth$', child)
                if match is None:
                    continue
                if match[1] is None:
        ...
        encoder_snapshot_path: Path = config.initial_model_prefix / f'encoder{infix}.pth'
        ...
        optimizer_snapshot_path: Path | None \
        ...
        scheduler_snapshot_path: Path | None \
    ...
    if config.loss_function not in ('l1', 'l2', 'smooth_l1', 'symlog'):
        raise RuntimeError(f'{config.loss_function}: An invalid value for `loss_function`.')
    ...
    if local_rank == 0:
        _config.device.dump(
            world_size=world_size, rank=rank, local_rank=local_rank, device=device, dtype=dtype,
            amp_dtype=amp_dtype)
    ...
    encoder_tdm = TensorDictModule(encoder, in_keys=['sparse', 'numeric'], out_keys=['encode'])
    decoder = Decoder(
        input_dimension=config.encoder.dimension, dimension=config.decoder.dimension,
        activation_function=config.decoder.activation_function, dropout=config.decoder.dropout,
        num_layers=config.decoder.num_layers, output_mode='scores', noise_init_std=0.0,
        device=torch.device('cpu'), dtype=dtype)
    ...
    identity_tdm = TensorDictModule(identity, in_keys=['decode'], out_keys=['grading_points'])
    symexp_module = Symexp()
    symexp_tdm = TensorDictModule(symexp_module, in_keys=['decode'], out_keys=['grading_points'])
    if config.loss_function == 'symlog':
        network_tdm_to_save = TensorDictSequential(encoder_tdm, decoder_tdm, symexp_tdm)
    else:
        network_tdm_to_save = TensorDictSequential(encoder_tdm, decoder_tdm, identity_tdm)
    ...
    def snapshot_writer(num_samples: Optional[int]=None) -> None:
        ...
        torch.save(network_tdm_state, snapshots_path / f'model{infix}.kanachan')

    tensorboard_path = output_prefix / 'tensorboard'
    tensorboard_path.mkdir(parents=True, exist_ok=True)
    with SummaryWriter(log_dir=tensorboard_path) as summary_writer:
        torch.autograd.set_detect_anomaly(False) # `True` for debbing purpose only.
        _training(
            device=device, dtype=dtype, amp_dtype=amp_dtype, training_data=config.training_data,
            num_workers=config.num_workers, num_samples=num_samples,
            celestial_scale=config.celestial_scale, network_tdm=network_tdm,
            batch_size=config.batch_size, loss_function=config.loss_function,
            gradient_accumulation_steps=config.gradient_accumulation_steps,
            max_gradient_norm=config.max_gradient_norm, optimizer=optimizer, scheduler=scheduler,
            snapshot_interval=config.snapshot_interval, summary_writer=summary_writer,
