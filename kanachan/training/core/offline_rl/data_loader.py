...
from kanachan.constants import (
    MAX_NUM_ACTIVE_SPARSE_FEATURES, NUM_NUMERIC_FEATURES, MAX_LENGTH_OF_PROGRESSION_FEATURES,
...
class DataLoader:
    def __init__(
            self, path: Path, get_reward: RewardFunction, *, batch_size: int=1, num_workers: int=0,
            pin_memory: bool=False, drop_last: bool=False) -> None:
    ...
    def __next__(self) -> TensorDict:
        ...
        with torch.no_grad():
            self.__get_reward(data)
        if data.get(('next', 'reward'), None) is None:
            raise RuntimeError("`get_reward` did not set the `reward` tensor.")
