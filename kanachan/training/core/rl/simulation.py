...
from kanachan.constants import (
    ...
    MAX_NUM_ACTION_CANDIDATES, NUM_TYPES_OF_ROUND_SUMMARY, MAX_NUM_ROUND_SUMMARY, RL_NUM_RESULTS)
...
@torch.no_grad()
def simulate(
        *, device: torch.device, dtype: torch.dtype, room: int, dongfengzhan: bool, grade: int,
        baseline_model: TensorDictModule, delete_baseline_keys: list[str],
        policy_model: TensorDictModule, delete_proposed_keys: list[str],
        num_simulations: int, simulation_batch_size: int, simulation_concurrency: int,
        progress: Progress, get_reward: RewardFunction) -> TensorDict:
    simulation_mode = 0
    ...
    game_logs = kanachan.simulation.simulate(
        device, dtype, room, grade, baseline_model, delete_baseline_keys, grade,
        policy_model, delete_proposed_keys, simulation_mode, num_simulations,
        simulation_batch_size, simulation_concurrency, progress)

    _episodes: [TensorDict] = []
    for game_log in game_logs:
        ...
        episode['next', 'sparse'][-1] = torch.full_like(sparse[-1], NUM_TYPES_OF_SPARSE_FEATURES)
        episode['next', 'numeric'] = numeric.roll(-1, 0).detach().clone()
        episode['next', 'numeric'][-1] = torch.full_like(numeric[-1], 0)
        episode['next', 'progression'] = progression.roll(-1, 0).detach().clone()
        ...
        episode['next', 'candidates'][-1] = torch.full_like(candidates[-1], NUM_TYPES_OF_ACTIONS)

        # TODO
        episode['next', 'round_summary'] = torch.full(
            (length, MAX_NUM_ROUND_SUMMARY), NUM_TYPES_OF_ROUND_SUMMARY,
            device=torch.device('cpu'), dtype=torch.int32)

        episode['next', 'results'] = torch.full(
            (length, RL_NUM_RESULTS), 0, device=torch.device('cpu'), dtype=torch.int32)
        ...
        for t in range(length):
            ...
            if round_key != next_round_key:
                episode['next', 'results'][t][0] = round_results[round_key]['delta_scores'][0]
                episode['next', 'results'][t][1] = round_results[round_key]['delta_scores'][1]
                episode['next', 'results'][t][2] = round_results[round_key]['delta_scores'][2]
                episode['next', 'results'][t][3] = round_results[round_key]['delta_scores'][3]
                episode['next', 'results'][t][4] = round_results[round_key]['scores'][0]
                episode['next', 'results'][t][5] = round_results[round_key]['scores'][1]
                episode['next', 'results'][t][6] = round_results[round_key]['scores'][2]
                episode['next', 'results'][t][7] = round_results[round_key]['scores'][3]
                episode['next', 'end_of_round'][t] = True
        episode['next', 'end_of_game'][-1] = True
        episode['next', 'done'] = episode['next', 'end_of_game'].detach().clone()

        get_reward(episode)
        if episode.get(('next', 'reward'), None) is None:
            raise RuntimeError("`get_reward` did not set the `('next', 'reward')` tensor.")
        reward: Tensor = episode['next', 'reward']
        if reward.dim() not in (1, 2):
            raise RuntimeError('An invalid shape of the `reward` tensor.')
        if reward.dim() == 2:
            if reward.size(1) != 1:
                raise RuntimeError('An invalid shape of the `reward` tensor.')
            reward.squeeze_(1)
        ...
        if reward.dtype not in (torch.float64, torch.float32, torch.float16):
            raise RuntimeError('An invalid `dtype` for the `reward` tensor.')

        _episodes.append(episode.detach().clone().cpu())

    episodes: TensorDict = torch.cat(_episodes).to_tensordict()
