...
def validate(config: Any):
    ...
    if config.optimizer.warmup_start_lr > config.optimizer.learning_rate:
        raise RuntimeError(
            f'{config.optimizer.warmup_start_lr}: `config.optimizer.warmup_start_lr` '
            'must be less than `config.optimizer.learning_rate`.')
...
def dump(config: Any) -> None:
    ...
    if config.optimizer.warmup_steps == 0:
        ...
        logging.info('LR warm-up start LR: %E', config.optimizer.warmup_start_lr)
        logging.info('LR warm-up steps: %d', config.optimizer.warmup_steps)
    if config.optimizer.annealing_steps == 0:
        logging.info('LR annealing: (disabled)')
    else:
        logging.info('LR annealing steps: %d', config.optimizer.annealing_steps)
        logging.info('LR annealing steps factor: %d', config.optimizer.annealing_steps_factor)
    logging.info('Use ZeRO: %s', config.optimizer.use_zero)


def create(config: Any, module: nn.Module) -> tuple[Optimizer, lr_scheduler.LRScheduler]:
    ...
    if config.optimizer.warmup_steps == 0:
        ...
        warmup_scheduler = lr_scheduler.LinearLR(
            optimizer,
            start_factor=config.optimizer.warmup_start_lr / config.optimizer.learning_rate,
