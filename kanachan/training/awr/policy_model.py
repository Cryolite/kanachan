...
from kanachan.nn import Encoder, LinearDecoder
...
class PolicyDecoder(nn.Module):
    ...
    def forward(self, candidates: torch.Tensor, encode: torch.Tensor) -> torch.Tensor:
        ...
        decode = self.linear_decoder(candidates, encode)
        prediction = nn.Softmax(dim=1)(decode)
        prediction = torch.where(candidates < NUM_TYPES_OF_ACTIONS, prediction, 0.0)
        assert prediction.dim() == 2
        assert prediction.size(0) == batch_size
