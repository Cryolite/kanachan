...
import kanachan.training.bc.config  # noqa: F401
...
def _main(config: DictConfig) -> None:
    ...
    if config.initial_model_prefix is not None:
        ...
        if config.initial_policy_decoder is not None:
            raise RuntimeError('`initial_model_prefix` conflicts with `initial_policy_decoder`.')
    ...
    decoder = Decoder(
        ...
        num_layers=config.decoder.num_layers, output_mode='candidates', noise_init_std=0.0,
        device=torch.device('cpu'), dtype=dtype)
    ...
    decode_converter_tdm = TensorDictModule(
        decode_converter, in_keys=['candidates', 'decode'], out_keys=['log_probs'])
    network_tdm = TensorDictSequential(encoder_tdm, decoder_tdm, decode_converter_tdm)
    ...
    network_tdm_to_save = TensorDictSequential(encoder_tdm, decoder_tdm, argmax_tdm)
