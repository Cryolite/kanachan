...
from kanachan.training.common import get_distributed_environment, get_gradient, is_gradient_nan
...
def _training(
        *, device: torch.device, dtype: torch.dtype, amp_dtype: torch.dtype,
        training_data: Path, num_workers: int, num_samples: int, network_tdm: nn.Module,
        batch_size: int, gradient_accumulation_steps: int, max_gradient_norm: float,
        optimizer: Optimizer, scheduler: LRScheduler | None, snapshot_interval: int,
    ...
    data_loader = DataLoader(
        path=training_data, batch_size=batch_size, num_workers=num_workers,
        pin_memory=(num_workers >= 1), drop_last=(world_size >= 2))
    ...
    for data in data_loader:
        ...
        ranking: Tensor = torch.zeros((batch_size,), device='cpu', dtype=torch.int64)
...
def _main(config: DictConfig) -> None:
    ...
    if config.initial_model_prefix is not None:
        ...
        if optimizer_snapshot_path is not None and (not optimizer_snapshot_path.is_file() or config.optimizer.initialize):
            optimizer_snapshot_path = None

        scheduler_snapshot_path = config.initial_model_prefix / f'lr-scheduler{infix}.pth'
        if scheduler_snapshot_path is not None and (not scheduler_snapshot_path.is_file() or config.optimizer.initialize):
    ...
    if local_rank == 0:
        ...
        if config.initial_model_prefix is not None:
            ...
            if scheduler_snapshot_path is not None:
                logging.info('Initial scheduler snapshot: %s', scheduler_snapshot_path)
    ...
    encoder_tdm = TensorDictModule(
        encoder, in_keys=['sparse', 'numeric', 'progression', 'candidates'], out_keys=['encode'])
    decoder = Decoder(
        ...
        num_layers=config.decoder.num_layers, output_mode='ranking', noise_init_std=0.0,
        device=torch.device('cpu'), dtype=dtype)
    decoder_tdm = TensorDictModule(decoder, in_keys=['encode'], out_keys=['decode'])
    ...
    softmax_tdm = TensorDictModule(softmax, in_keys=['decode'], out_keys=['ranking_probs'])
    network_tdm_to_save = TensorDictSequential(encoder_tdm, decoder_tdm, softmax_tdm)
    ...
    with SummaryWriter(log_dir=tensorboard_path) as summary_writer:
        ...
        _training(
            ...
            num_workers=config.num_workers, num_samples=num_samples, network_tdm=network_tdm,
